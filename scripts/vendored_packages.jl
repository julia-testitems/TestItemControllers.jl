...
`testprocess/TestItemServer/src/pkg_imports.jl` picks between. Adding an entry here and
running `update_vendored_packages.jl --apply` vendors it.
"""
const VENDORED = [
    ...
    "packages/CoverageTools"               => "JuliaCI/CoverageTools.jl",
    "packages/DebugAdapter"                => "julia-vscode/DebugAdapter.jl",
    ...
    "packages/URIParser"                   => "JuliaWeb/URIParser.jl",
...
const FROZEN = Dict(
    ...
    "packages/CoverageTools" =>
        "newer versions depend on JuliaSyntax, which would have to be vendored too",
    "packages/Compiler" =>
